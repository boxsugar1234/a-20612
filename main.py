...
# ---------------------------------------------------------
# 여덟 번째 구역: 국가 및 영화별 총 관객 수 트리맵 그래프
# ---------------------------------------------------------
title_q8 = (
    "8. 각 국가에서 어떤 영화가 가장 흥행했는가. 그리고 대체로 어떤 국가에서 나온"
    " 영화들의 관객 수가 많은가."
)
st.subheader(title_q8)

# 플롯리 트리맵 그래프 생성 (계층: 제작 국가 -> 영화명, 크기: 총 관객 수)
fig8 = px.treemap(
    df,
    path=[px.Constant("전체"), "nation", "movieNm"],
    values="total_audi",
    title=(
        "각 국가에서 어떤 영화가 가장 흥행했는가. 그리고 대체로 어떤 국가에서 나온"
        " 영화들의 관객 수가 많은가."
    ),
    hover_data={"total_audi": ":,d"},
)
...
st.markdown("---")
st.markdown(
    "**이 그래프로 알 수 있는 것:** 국가 전체 블록 크기를 통해 어떤 국가 제작 영화들이 박스오피스 총 관객 수를 주도했는지 비교하고, 국가 내 개별 영화 영역 크기로 해당 국가의 흥행 리드 작품을 파악할 수 있습니다."
)
